...
int main(void) {
  ...
  for(;;) {
    ...
    if (testFichierVide(fichierBall)){
      char pasBallon[256] = "pas de ballon";
      send(client_sock, pasBallon,256, 0);
    }
    else{
      char BallonEnvoi[256];
      fgets(BallonEnvoi, 256, fichierBall);
      send(client_sock, BallonEnvoi, 256, 0);
    }
     fclose(fichierBall);
    /* Reception de donnees du client */
     char buffer[256];
     printf("attente\n");
    int res = recv(client_sock, buffer, 256, 0);
    printf("recv %s\n", buffer);
